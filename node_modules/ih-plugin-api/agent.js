...
function Agent(params, connector) {
  ...
  this.connector.on('scan', message => {
    if (that.handlers.onScan) that.handlers.onScan(message);
  });
...
// НЕ ИСПОЛЬЗУЕТСЯ, решается на уровне плагина (index)
...
Agent.prototype.sendRequest = function(type, data) {
  ...
  connector.send(sobj);
  const type_id = type + '_' + id;
  return new Promise((resolve, reject) => {
    connector.once(type_id, message => {
      if (message.response) {
...
Agent.prototype.processSubResponse = function(message) {
  const id = message.id;
  try {
    if (!id) throw { message: 'Expected id!' };

    if (!this.subs[id]) throw { message: 'Not found subs!' };
