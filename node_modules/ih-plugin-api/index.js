...
function Plugin(agent) {
  ...
  this.devhard = apiobj('devhard', this);
...
Plugin.prototype.sleep = async function (ms) {
...
Plugin.prototype.sendWorkingState = function() {
  this.agent.send({ type: 'state', data: 1 });
  // this.log('SendData ' + util.inspect(data), 2);
};
...
Plugin.prototype.get = function(name, filter, opt) {
  return this.agent.sendRequest('get', { name, filter, opt });
};
