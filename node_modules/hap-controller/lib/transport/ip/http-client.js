...
class HttpClient extends events_1.EventEmitter {
    ...
    async setCharacteristics(characteristics) {
        ...
        if (this.isInit || this.isComplete) {
            this.isInit = false;
            const connection = this.setscriptionConnection || new http_connection_1.default(this.address, this.port);
            const data = {
                characteristics: [],
            };
            const keys = this.setkeys || await this._pairVerify(connection);

            if (this.isComplete == false) {
             this.setscriptionConnection = connection;
             this.setkeys = keys;
              this.isComplete = true;
              this.buffer.forEach(i => {
                this.setCharacteristics(i);
              });
              this.buffer = [];
            }
         ...
         this.buffer.push(characteristics)
        }
    }
    /**
    ...
    async subscribeCharacteristics(characteristics) {
        const connection = this.subscriptionConnection || new http_connection_1.default(this.address, this.port);
        const data = {
            characteristics: [],
        };
        const keys = await this._pairVerify(connection);
