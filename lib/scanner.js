...
class Scanner {
  ...
  request(scanObj) {
    ...
    this.plugin.log('Scan Request scanObj.uuid='+scanObj.uuid);
  ...
  start() {
    ...
    setTimeout(() => {
      ...
      // this.plugin.log('Scan sendTree FIRST: '+util.inspect(data, null, 8));
  ...
  process(topic, message, titles) {
   
    const result = this.scanTree.add(topic, message, titles);
    if (this.status == 2) {
      // Дослать изменения
      const resObj = {};
      if (result.added) {
        ...
        if (!this.updateMap.size) {
          setTimeout(() => {
            ...
            // const data = Array.from(this.updateMap.values());
             this.plugin.log('Scan sendTree NEXT: '+util.inspect(data, null, 8));
